...
def walk_category_folder(path): # needs edits if direct from source, run "walk" first
    create_editted_directory(path)

    rename_or_delete_files(path)

def walk_resource_folder(path): # needs edits if direct from source, run "walk" first
    rename_dirs(path)
